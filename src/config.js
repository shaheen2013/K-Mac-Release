...
function getConfigDir() {
    ...
    if (platform === 'win32') {
        // Windows: %APPDATA%\klevere-config
        configDir = path.join(os.homedir(), 'AppData', 'Roaming', 'klevere-config');
    } else if (platform === 'darwin') {
        // macOS: ~/Library/Application Support/klevere-config
        configDir = path.join(os.homedir(), 'Library', 'Application Support', 'klevere-config');
    } else {
        // Linux and others: ~/.config/klevere-config
        configDir = path.join(os.homedir(), '.config', 'klevere-config');
    }
