...
async function initializeGeminiSession(apiKey, customPrompt = '', profile = 'interview', language = 'en-US', isReconnection = false) {
    ...
    try {
        const ws = new WebSocket(process.env.WEBSOCKET_URL || 'wss://app.klevere.ai:5000/ws');
        ...
        ws.on('message', raw => {
            try {
                ...
                if (msg.type === 'model' && Array.isArray(msg.parts)) {
                ...
                if (msg.type === 'transcription' && msg.results) {
                    currentTranscription += formatSpeakerResults(msg.results);
                }
                ...
                if (msg.type === 'error') {
                    const eMsg = msg.message || 'Unknown error';
                    const isApiKeyError =
                        eMsg.includes('API key not valid') ||
                        eMsg.includes('invalid API key') ||
                    ...
                    sendToRenderer('update-status', 'Error: ' + eMsg);
                }
            } catch (e) {
        ...
        const session = {
            async sendRealtimeInput(payload) {
        ...
        console.error('Failed to initialize Gemini session (WS):', error);
        isInitializingSession = false;
        sendToRenderer('session-initializing', false);
        return null;
    }
...
function setupGeminiIpcHandlers(geminiSessionRef) {
    ...
    ipcMain.handle('pause-audio-stream', async event => {
        try {
            isAudioPaused = true;
            return { success: true };
        } catch (error) {
    ...
    ipcMain.handle('resume-audio-stream', async event => {
        try {
            isAudioPaused = false;
            return { success: true };
        } catch (error) {
