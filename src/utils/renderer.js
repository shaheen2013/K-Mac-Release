...
// Listen for responses - REMOVED: This is handled in KlevereApp.js to avoid duplicates
...
const klevereApp = document.querySelector('klevere-app');
...
const cheddar = {
    ...
    getCurrentView: () => klevereApp.currentView,
    getLayoutMode: () => klevereApp.layoutMode,

    // Status and response functions
    setStatus: text => klevereApp.setStatus(text),
    setResponse: response => klevereApp.setResponse(response),
    ...
    pauseAudio: async () => {
        audioPaused = true;
        try { await ipcRenderer.invoke('pause-audio-stream'); } catch {}
        console.log('Audio paused');
    },
    resumeAudio: async () => {
        audioPaused = false;
        try { await ipcRenderer.invoke('resume-audio-stream'); } catch {}
