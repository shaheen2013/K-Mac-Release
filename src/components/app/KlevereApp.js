...
export class KlevereApp extends LitElement {
    static styles = css`
        * {
    ...
    setStatus(text) {
        ...
        if (this.currentView === 'research' && this.researchLoading) {
            console.log('Research status:', text);
        }
    }

    setResponse(response) {
        // Handle research responses - check if we're in research mode and loading
        if (this.currentView === 'research' && this.researchLoading) {
            // Accumulate the response for research
            this.researchResults = response;
            
            // Check if response seems complete (has substantial content and ends properly)
            const isComplete = response.length > 200 && 
                              (response.includes('##') || response.includes('**') || response.includes('1.') || response.includes('2.')) &&
                              (response.endsWith('.') || response.endsWith('!') || response.endsWith('?'));
            
            if (isComplete) {
                this.researchLoading = false;
                console.log('Research response complete');
            }
        ...
        // If we're in research mode but not loading, don't add to main chat
        if (this.currentView === 'research') {
            return;
        }
    ...
    async handleResearchSubmit(url) {
        this.researchLoading = true;
        this.researchResults = '';
        this.requestUpdate();

        // Set a timeout to ensure research completes
        const researchTimeout = setTimeout(() => {
            if (this.researchLoading) {
                console.log('Research timeout reached, marking as complete');
                this.researchLoading = false;
                this.requestUpdate();
            }
        ...
        try {
            // Check if we have an active session, if not create a temporary one for research
            if (!window.cheddar || !window.cheddar.sendTextMessage) {
                // Initialize a temporary session for research
                const apiKey = localStorage.getItem('apiKey')?.trim();
                if (!apiKey) {
                    throw new Error('API key not found. Please configure your API key in settings.');
                }

                // Initialize Gemini session for research
                await window.cheddar.initializeGemini('interview', 'en-US');
            }

            // Use the existing socket connection to send research request
            const result = await window.cheddar.sendTextMessage(
                `Research this website for sales insights: ${url}. Provide exactly 5 key points that will help a salesperson understand 
                and personalize their pitch to this lead. Focus on: recent funding/news, company size/stage, key pain points, decision makers, 
                and specific value propositions. Keep each point concise (1-2 sentences max). Format as numbered list. Do not include any other 
                text or formatting, maximum 500 characters`
            );

            if (!result.success) {
                throw new Error(result.error || 'Failed to research website');
            }

            // Clear timeout when we get a successful response
            clearTimeout(researchTimeout);

            // The response will come through the normal response system
            // We'll handle it in the setResponse method
        } catch (error) {
            console.error('Research failed:', error);
            clearTimeout(researchTimeout);
            this.researchLoading = false;
            this.researchResults = `Error: ${error.message}`;
            this.requestUpdate();
        }
    }

    async handleToggleAudio() {
        // Toggle pause/resume of audio streaming without closing the session
        this.isAudioPaused = !this.isAudioPaused;

        // Notify renderer utilities
        if (this.isAudioPaused) {
            window.cheddar?.pauseAudio?.();
            this.setStatus('Audio paused');
        } else {
            window.cheddar?.resumeAudio?.();
            this.setStatus('Listening');
        }

        // Persist preference for current run only
        this.requestUpdate();
    }

    async handleClose() {
        if (this.currentView === 'customize' || this.currentView === 'help' || this.currentView === 'history' || this.currentView === 'research') {
    ...
    async handleAPIKeyHelp() {
        if (window.require) {
            const { ipcRenderer } = window.require('electron');
            await ipcRenderer.invoke('open-external', 'https://klevere.ai/help/api-key');
        }
    }
            ...
            case 'research':
                return html`
                    <research-view
                        ...
                        .onResearchSubmit=${url => this.handleResearchSubmit(url)}
                        .isLoading=${this.researchLoading}
            ...
            <div class="window-container">
                <div class="container">
                    <app-header
                        ...
                        .onBackClick=${() => this.handleBackClick()}
                        .onToggleAudioClick=${() => this.handleToggleAudio()}
...
customElements.define('klevere-app', KlevereApp);
